...
file.gene_prioritization <- "../gene_lists/release_v1/gene_prioritization.txt"
#file.gene_prioritization <- "../gene_lists/release_v1/gene_associated.txt"
...
#file.depict.gene_set <- "../data/release_v1/noindels.subset.Plog5.N50k.meta.rs_p1e-5_sign_genesets.tab"
...
#save(df.depict.gene_set, file="RData/df.depict_gene_release_v1_02-20-2015.RData")

### Load RData
load(file="RData/df.depict_gene_release_v1_02-20-2015.RData") # df.depict.gene_set
...
x.intersection <- read.table(text="Dendrite
Axon
ATXN1.protein.complex
Dendritic.Shaft
Dendritic
Synapse
Neuron.Spine
DLG3.protein.complex")

df.correlation.extract.stage %>% filter(!gene_set %in% x.intersection$V1) %>% select(gene_set) %>% distinct()
p <- ggplot(df.correlation.extract.stage %>% filter(!gene_set %in% x.intersection$V1), aes(x=stage, y=cor.estimate)) + geom_boxplot()
p
